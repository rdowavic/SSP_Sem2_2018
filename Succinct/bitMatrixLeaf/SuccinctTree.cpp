    ...
    SuccinctTree result(dim); result.giveChild();
		...
		if (dim == LEAF_MUM) {
			for (size_t i = 0; i < NUM_CHILDREN; ++i) {
				result.set(HEAD_NODE, i, A.ruler[nodeA + i] | B.ruler[nodeB + i]);		
			}
			return result;
		}
...
SuccinctTree SuccinctTree::Product(const SuccinctTree& A, Node nodeA,
    ...
    SuccinctTree result(dim); result.giveChild();
		...
		if (dim == LEAF_MUM) {
            ...
            entry |= mult(A.ruler[nodeA + ROW_SIZE*i + k], B.ruler[nodeB + ROW_SIZE*k + j]);
//						entry |= mult(A.get(nodeA, i, k), B.get(nodeB, k, j));
					}
				 result.set(HEAD_NODE, i, j, entry);
			  }
			}
      // it should not be completely empty... ?
      // check if it represents the zero matrix right now
      bool seenNonZero = false;
      for (int i = 0; i < 4; ++i) { if (result.ruler[i] != 0) seenNonZero = true; }
      if (!seenNonZero) {
        result.ruler.clear();
      }
			return result;
...
// get returns a NODE, NOT AN ENTRY!!!!!!!!!!!
